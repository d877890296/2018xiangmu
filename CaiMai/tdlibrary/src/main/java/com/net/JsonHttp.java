...
import com.net.http.AjaxParams;
import com.net.http.HttpHandler;
import com.net.http.RetryHandler;
import com.net.http.SyncRequestHandler;

public class JsonHttp {

	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 20 * 1024; //20KB
	...
	private static final ThreadFactory sThreadFactory = new ThreadFactory() {
		...
		public Thread newThread(Runnable r) {
			Thread tread = new Thread(r, "BtHttp #" + mCount.getAndIncrement());
	...
	public void getJson(String url, Class<?> clazz, AjaxCallBack<? extends Object> callBack) {
		new HttpHandler(httpClient, httpContext, callBack, charset).executeOnExecutor(executor,
				new HttpGet(getUrlWithQueryString(url, null)), "json", clazz);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void getJsonObj(String jsonObj, Class<?> clazz, AjaxCallBack<? extends Object> callBack) {
		new HttpHandler(httpClient, httpContext, callBack, charset).executeOnExecutor(executor,
				jsonObj, "jsonObj", clazz);
	}
	...
	public void postJson(String url, AjaxParams params, Class<?> clazz, AjaxCallBack<? extends Object> callBack) {
		new HttpHandler(httpClient, httpContext, callBack, charset).executeOnExecutor(executor,
				addEntityToRequestBase(new HttpPost(url), paramsToEntity(params)), "json", clazz);
	}
	...
	public HttpHandler<File> download(String url, AjaxParams params, String target, boolean isResume,
		...
		handler.executeOnExecutor(executor, get, "file", target, isResume);
		return handler;
	}
