...
/***
 * 
 * com.dev.refushView.RefreshLayout
 * 
 * @author lyy
 *
 */
...
public class RefreshLayout extends LinearLayout {
	...
	/***
	 * 加载更多的条件限制 默认10条 小于(10-1)条数据就不能加载更多
	 */
	private int limitMoreLoadCount = 10;
	 ...
	 * 加载更多的条件限制 默认10条 小于(10-1)条数据就不能加载更多
	 * 
	 * @param limitMoreLoadCount
	 */
	public void setLimitMoreLoadCount(int limitMoreLoadCount) {
		if (limitMoreLoadCount<0||(limitMoreLoadCount == 0 && limitMoreLoadCount < 10)) {
			limitMoreLoadCount = 10;
		}
		this.limitMoreLoadCount = (limitMoreLoadCount - 1);
	}
	...
	private void initRefreshHeaderView() {
		...
		if (mRefreshHeaderView != null) {
			...
			mMaxWholeHeaderViewPaddingTop = (int) (mRefreshHeaderViewHeight
					* mRefreshViewHolder.getSpringDistanceScale());
		...
		if (mRecyclerView != null) {
			mRecyclerView.setOnScrollListener(new RecyclerView.OnScrollListener() {
				...
				public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
					//Logger.e("-----------newState-------", newState + "-newState------------");
	...
	private boolean shouldHandleAbsListViewLoadingMore() {
		if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING || mLoadMoreFooterView == null
				|| mDelegate == null || mAbsListView.getAdapter() == null
	...
	private boolean shouldHandleRecyclerViewLoadingMore() {
        ...
        if (manager == null || (manager.getItemCount() >= 0&&manager.getItemCount() <=limitMoreLoadCount)) {
            return false;
        }
	...
	private boolean shouldHandleLoadingMore() {
		...
		if (mWebView != null) {
			if (mWebView.getContentHeight()
					* mWebView.getScale() == (mWebView.getScrollY() + mWebView.getMeasuredHeight())) {
				return true;
			}
		}

		if (mScrollView != null) {
			int scrollContentHeight = mScrollView.getScrollY() + mScrollView.getMeasuredHeight()
	...
	private boolean shouldHandleRefresh() {
		if (mIsLoadingMore || mCurrentRefreshStatus == RefreshStatus.REFRESHING || mRefreshHeaderView == null
				|| mDelegate == null) {
			return false;
		}
	...
	private boolean handleActionUpOrCancel(MotionEvent event) {
		...
		if (mCurrentRefreshStatus == RefreshStatus.PULL_DOWN || mCurrentRefreshStatus == RefreshStatus.IDLE) {
			...
			if (mCustomHeaderView == null || (mCustomHeaderView != null && mWholeHeaderView.getPaddingTop() < 0
					&& mWholeHeaderView.getPaddingTop() > mMinWholeHeaderViewPaddingTop)) {
